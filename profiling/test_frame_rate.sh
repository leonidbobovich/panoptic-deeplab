...
[ "${NOTRANDOM}" == "1" ] && CALIBRATION_FILE_LIST=$(python -c "import onnx; model = onnx.load('${MODEL}'); print('files/0.txt' if model.graph.input[0].type.tensor_type.shape.dim[1].dim_value < model.graph.input[0].type.tensor_type.shape.dim[3].dim_value else 'files/1.txt')") || CALIBRATION_FILE_LIST=""
...
[ "${QUANTIZE}" == "8" ] && [ ${CALIBRATE} == "1" ] && Q_FLAGS="${Q_FLAGS} -profiling-threads=128 -compile-threads=128"
[ "${CALIBRATION_FILE_LIST}" != "" ] && Q_FLAGS="${Q_FLAGS} -input-list-file=/home/ubuntu/${CALIBRATION_FILE_LIST}"
...
#-aic-perf-warnings -aic-perf-metrics
COMPILE_FLAGS="-m=${REMOTE_BUILD_DIR}/${MD5_MODEL}.onnx -aic-hw -aic-hw-version=2.0 -aic-num-cores=1 -mos=1 -batchsize=1 \
-onnx-define-symbol=batch,1 -onnx-define-symbol=unk__80,1 -time-passes -time=60 -device-id=0  \
${Q_FLAGS}\
 "
...
date && \
${QAIC_EXEC} "find ${REMOTE_BUILD_DIR}/${COMPILE_MD5}/output -type f -delete" && \
${QAIC_EXEC} "cd ${REMOTE_BUILD_DIR}/${COMPILE_MD5} && tar  czf - ." | ( cd ${LOCAL_BUILD_DIR}/${COMPILE_MD5} && tar xf - ) && \
date

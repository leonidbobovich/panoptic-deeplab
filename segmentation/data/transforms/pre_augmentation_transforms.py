...
class Resize(object):
    ...
    def __call__(self, image, label):
        ...
        # cv2: (width, height)
        image = cv2.resize(image.astype(np.float64), (new_size[1], new_size[0]), interpolation=cv2.INTER_LINEAR)
        label = cv2.resize(label.astype(np.float64), (new_size[1], new_size[0]), interpolation=cv2.INTER_NEAREST)
